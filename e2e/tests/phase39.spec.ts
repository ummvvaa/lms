 ...
 * С фазы 48 школа показывает два экзамена — SAT и IELTS; скрытый
 * в списке целей не появляется, архивный (фаза 59) — тем более.
 */
...
test("ученик ставит цель IELTS с датой — строка уходит на проверку", async ({
  ...
  // скрыты признаком показа у записи справочника. Архивный экзамен (фаза 59)
  // не появляется нигде, даже если бы его включили галочкой
  await expect(goals.locator('[data-exam="SAT"]')).toBeVisible();
  await expect(goals.locator('[data-exam="TOEFL"]')).toHaveCount(0);
