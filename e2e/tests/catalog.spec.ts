...
test.describe('каталог глазами ученика', () => {
  ...
  test('карточка показывает процент, разбивку и конкретный разрыв', async ({ page }) => {
    ...
    await expect(card.locator('.match__caption')).toContainText('соответствие требованиям')
  ...
  test('у программы с порогом IELTS выше текущего виден разрыв 0.5 IELTS', async ({ page, browser }) => {
    ...
    const withGap = results.find((card: { summary: string }) => card.summary.includes('0.5 IELTS'))
    expect(withGap, 'у ученика с IELTS 6.0 должна найтись программа с порогом 6.5').toBeTruthy()
    expect(withGap.percent).toBeGreaterThan(0)
    expect(withGap.percent).toBeLessThanOrEqual(100)

    const english = withGap.breakdown.find((p: { code: string }) => p.code === 'english')
    expect(english.gap_phrase).toContain('0.5 IELTS')

    await page.goto('/catalog')
    await page.locator('.input').first().fill(withGap.university_name)
    const card = page.locator('.match').filter({ hasText: withGap.university_name })
  ...
  test('нигде не обещается шанс поступления', async ({ page }) => {
    for (const route of ['/catalog', '/universities']) {
      ...
      expect(text, `на ${route} проценту дано запрещённое название`).not.toMatch(FORBIDDEN)
    }
  })

  test('«что откроется, если» пересчитывает список по ползункам', async ({ page }) => {
    await page.goto('/catalog')
    ...
    await expect(page.locator('.chip-ok').first()).toContainText('Проходите полностью')
...
test.describe('подбор словами', () => {
  ...
  test('называет только программы справочника', async ({ page }) => {
    ...
    const names: string[] = known.results.map((c: { university_name: string }) => c.university_name)
    ...
    for (const pick of payload.picks) {
      expect(names, `${pick.university_name} нет в справочнике`).toContain(pick.university_name)
    }
    expect(JSON.stringify(payload)).not.toMatch(FORBIDDEN)
  })

  test('по стране, которой нет в справочнике, честно говорит об этом', async ({ page }) => {
    await page.goto('/catalog')
    ...
    await expect(page.locator('.catalog__note')).toContainText('нет программ по запросу')
...
test.describe('добавление в свой список', () => {
  test('ученик добавляет target, Асем видит это на подтверждении', async ({ browser }) => {
    ...
    const { results } = await (await student.request.get('/api/catalog/')).json()
    const target = results.find((c: { in_my_list: boolean }) => !c.in_my_list)
    expect(target, 'нужна программа вне списка ученика').toBeTruthy()

    await student.locator('.input').first().fill(target.university_name)
    const card = student.locator('.match').filter({ hasText: target.university_name }).first()
    await expect(card).toBeVisible()
    await expect(card.getByRole('button', { name: 'Добавить к себе' })).toBeVisible()
    ...
    await expect(student.locator('.match').filter({ hasText: target.university_name })).toContainText(
      'ждёт подтверждения',
    )
    ...
    const queue = director.locator('.card').filter({ hasText: 'Ученики добавили себе' })
    await expect(queue).toContainText(target.university_name)

    const [confirmed] = await Promise.all([
      director.waitForResponse((r) => r.url().includes('/api/catalog/pending/')),
    ...
    await expect(student.locator('.match').filter({ hasText: target.university_name })).not.toContainText(
      'ждёт подтверждения',
    )

    // прибираем за собой: иначе следующий прогон не найдёт свободной программы
    const csrf = (await directorContext.cookies()).find((c) => c.name === 'csrftoken')!.value
    const entry = await (
      await director.request.get(`/api/student-universities/?program=${target.program}`)
    ).json()
    const mine = (entry.results ?? entry).find((row: { program: number }) => row.program === target.program)
    if (mine) {
  ...
  test('добавленное директором ученик снять не может', async ({ page, request }) => {
    ...
    const byDirector = results.find(
      (c: { my_entry: { added_by: string } | null }) => c.my_entry && c.my_entry.added_by === 'director',
    )
    expect(byDirector, 'нужна запись, добавленная директором').toBeTruthy()

    const csrf = (await context.cookies()).find((c) => c.name === 'csrftoken')!.value
    const denied = await student.request.delete(`/api/catalog/remove/${byDirector.my_entry.id}/`, {
      headers: { 'X-CSRFToken': csrf },
    })
