...
const SCREENS: Record<string, string[]> = {
  director_behavior: ['/dashboard', '/table', '/import', '/assistant', '/suggestions', '/digest', '/alumni'],
  director_admission: ['/dashboard', '/directory', '/table', '/import', '/suggestions'],
  director_exam: ['/dashboard', '/table', '/import', '/digest'],
  director_talent: ['/dashboard', '/table'],
  director_sport: ['/dashboard', '/table'],
  admin: ['/dashboard', '/users', '/archive', '/table'],
  student: ['/dashboard', '/roadmap', '/universities', '/catalog', '/prep', '/essays', '/alumni'],
}
  ...
  for (const size of SIZES) {
    test(`снимки ${role} ${size.name}`, async ({ browser }) => {
      ...
      for (const screen of screens) {
        ...
        await page.screenshot({ path: path.join(DIR, size.name, name), fullPage: true })
      }
      await context.close()
    })
  }
...
test('снимок первого входа', async ({ browser }) => {
  const context = await browser.newContext({ storageState: statePath('director_talent') })
  const page = await context.newPage()
  await page.addInitScript(() => window.localStorage.clear())
  await page.goto('/dashboard')
  await page.waitForTimeout(600)
  fs.mkdirSync(path.join(DIR, 'wide'), { recursive: true })
  await page.screenshot({ path: path.join(DIR, 'wide', 'first-run.png'), fullPage: true })
  await context.close()
})
