...
test.describe("таблица директора", () => {
  ...
  test("нечисловое значение отклоняется внятно, а не пятисоткой", async ({
    page,
  }) => {
  ...
  test("в таблицу попадают все ученики, а не первая страница", async ({
    page,
  }) => {
...
test.describe("карточка ученика", () => {
  ...
  test("правка на карточке пишется в историю и остаётся после перезагрузки", async ({
    page,
  }) => {
    const list = await (
    ...
    expect(response.request().postDataJSON().changes[0]).toHaveProperty(
      "expected",
      before,
    );
    ...
    await page.reload();
    await expect(
      page.locator(".domain--mine input.domain__input").nth(4),
    ).toHaveValue(value);
