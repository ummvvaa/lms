 ...
 */
async function setPasswordByLink(browser: Browser, email: string): Promise<void> {
...
test('сквозной путь: от пустой базы до возврата ученика из архива', async ({ browser }) => {
  const notes: string[] = []
  ...
  const emptyStudents = await (await adminPage.request.get('/api/students/?page_size=1')).json()
  expect(emptyStudents.count, 'после очистки учеников быть не должно').toBe(0)
  const emptyCatalog = await (await adminPage.request.get('/api/universities/?page_size=1')).json()
  ...
  const madeDirector = await adminPage.request.post('/api/users/', {
    data: { email: NEW_DIRECTOR, full_name: 'Асем (сквозной)', role: 'director_admission' },
    headers: { 'X-CSRFToken': csrf },
  })
  expect(madeDirector.ok(), 'директор должен заводиться из интерфейса').toBeTruthy()
  ...
  await adminPage.getByLabel('Куратор группы').fill('Салтанат')
  ...
  const card = await (await adminPage.request.get(`/api/students/${studentId}/`)).json()
  ...
  await login(directorPage, { key: 'x', email: NEW_DIRECTOR, password: NEW_PASSWORD, title: 'Асем' })

  await directorPage.goto('/directory')
  await expect(directorPage.locator('.dir__empty')).toBeVisible()
  await directorPage.getByRole('button', { name: 'Заполнить стартовый справочник' }).click()
  await expect
    .poll(async () => (await (await directorPage.request.get('/api/catalog/seed/')).json()).universities)
    .toBe(20)
  ...
  await directorPage.reload()
  const verifiedRow = directorPage.locator('.dir__row').filter({ hasText: verifiedName }).first()
  await expect(verifiedRow.getByText('подтверждено', { exact: true })).toBeVisible()

  // --- 4. Ученик проходит онбординг-квиз --------------------------------
  const learner = await browser.newContext()
  const learnerPage = await learner.newPage()
  await login(learnerPage, { key: 'x', email: NEW_STUDENT, password: NEW_PASSWORD, title: 'Алия' })
  ...
  for (let step = 0; step < 20; step += 1) {
    if (await learnerPage.getByRole('button', { name: 'В кабинет' }).count()) break
  ...
  const progress = await (await learnerPage.request.get('/api/onboarding/')).json()
  ...
  const profileAfterQuiz = await (await learnerPage.request.get('/api/students/me/')).json()
  expect(profileAfterQuiz.exam, 'кабинет наполняется ответами квиза').toBeTruthy()
  ...
  const mine = await (await learnerPage.request.get('/api/student-universities/?page_size=50')).json()
  expect(mine.count, 'два вуза должны лечь в список ученика').toBeGreaterThanOrEqual(2)

  // --- 6. Подбор через ИИ: только вузы справочника -----------------------
  const catalogNames = (await (await learnerPage.request.get('/api/universities/?page_size=100')).json()).results.map(
  ...
  for (const row of pick.picks ?? []) {
    expect(catalogNames, `подбор назвал вуз не из справочника: ${row.university_name}`).toContain(
      row.university_name,
    )
  }
  ...
  const pending = directorPage.locator('.card').filter({ hasText: 'Ученики добавили себе' })
  ...
  const upload = async (value: string, name: string) => {
    ...
    await mapping.nth(1).locator('select').selectOption({ label: 'IELTS текущий' })
    await examPage.getByRole('button', { name: 'Показать предпросмотр' }).click()
    await examPage.getByRole('button', { name: /Применить/ }).click()
    await examPage.waitForTimeout(600)
  }

  await upload('7.5', 'баллы.csv')
  let profile = await (await examPage.request.get(`/api/profiles/exam/${studentId}/`)).json()
  expect(profile.ielts_current).toBe('7.5')

  await examPage.reload()
  const batch = examPage.locator('.imp__row').filter({ hasText: 'баллы.csv' }).first()
  await batch.getByRole('button', { name: 'Отменить импорт' }).click()
  await examPage.locator('.confirm').getByRole('button', { name: 'Отменить импорт' }).click()
  await expect(examPage.locator('.imp__report')).toContainText('Возвращено прежних значений')

  profile = await (await examPage.request.get(`/api/profiles/exam/${studentId}/`)).json()
  expect(profile.ielts_current, 'отмена импорта должна вернуть прежнее значение').not.toBe('7.5')
  ...
  await expect(adminPage.locator('.confirm')).toContainText('Вместе с записью уйдёт связанное')
  await adminPage.getByLabel('Наберите УДАЛИТЬ').fill('УДАЛИТЬ')
  await adminPage.locator('.confirm').getByRole('button', { name: 'Удалить', exact: true }).click()
  await adminPage.waitForURL(/\/table/)

  const afterDelete = await (await adminPage.request.get('/api/students/?page_size=10')).json()
  expect(afterDelete.count, 'удалённый ученик исчезает из списков').toBe(0)

  await adminPage.goto('/archive')
  const entry = adminPage.locator('.arch__row').filter({ hasText: 'Ахметова Алия' }).first()
  await expect(entry).toBeVisible()
  await entry.getByRole('button', { name: 'Восстановить' }).click()
  await expect(adminPage.locator('.arch__flash')).toContainText('Восстановлено записей')

  const afterRestore = await (await adminPage.request.get('/api/students/?page_size=10')).json()
  expect(afterRestore.count, 'из архива ученик возвращается').toBe(1)
  const backUniversities = await (
    await adminPage.request.get(`/api/student-universities/?student=${studentId}&page_size=50`)
  ).json()
  expect(backUniversities.count, 'связи возвращаются вместе с учеником').toBeGreaterThanOrEqual(2)

  for (const context of [admin, director, learner, examContext]) await context.close()
