...
test.describe("справочник у директора по поступлению", () => {
  ...
  test("кнопка заводит стартовый справочник, и он виден после перезагрузки", async ({
    page,
  }) => {
      ...
      .poll(
        () =>
          diag
            .since(mark)
            .filter(
              (c) => c.method === "POST" && c.url.includes("/catalog/seed/"),
            ).length,
      )
      .toBeGreaterThan(0);
  ...
  test("«Подтвердить данные» снимает плашку и держится после перезагрузки", async ({
    page,
  }) => {
    const diag = watch(page);
    await page.goto("/directory");
      ...
      .poll(
        () =>
          diag
            .since(mark)
            .filter(
              (c) => c.method === "POST" && c.url.includes("/catalog/verify/"),
            ).length,
      )
      .toBeGreaterThan(0);
    expect(diag.failed).toEqual([]);
  ...
  test("удаление заготовки не трогает вуз, заведённый школой", async ({
    page,
  }) => {
    const diag = watch(page);
    await page.goto("/directory");
...
test.describe("плашка глазами ученика", () => {
  test("непроверенная программа приходит ученику только с оговоркой", async ({
    browser,
  }) => {
