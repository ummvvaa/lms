...
test.describe('поиск по системе', () => {
  ...
  test('находит ученика по фамилии и открывает его карточку', async ({ page }) => {
    ...
    await expect(drop.locator('.search__grouptitle').first()).toContainText('Ученики')

    const row = drop.locator('.search__row').filter({ hasText: student.full_name }).first()
  ...
  test('ничего не найдя, объясняет это словами', async ({ page }) => {
    ...
    await expect(page.locator('.search__empty')).toContainText('Ничего не нашлось')
  })
})
...
test.describe('поиск глазами ученика', () => {
  ...
  test('ученик не находит одноклассников — только вузы', async ({ browser, page }) => {
    const staff = await browser.newContext({ storageState: statePath('director_exam') })
    const staffPage = await staff.newPage()
    await staffPage.goto('/dashboard')
    const list = await (await staffPage.request.get('/api/students/?page_size=1')).json()
    const surname = list.results[0].full_name.split(' ')[0]
    await staff.close()

    // сырой ответ API: интерфейс мог бы просто не нарисовать группу
    const payload = await (await page.request.get(`/api/search/?q=${encodeURIComponent(surname)}`)).json()
    expect(payload.groups.map((g: { code: string }) => g.code)).not.toContain('students')
...
test.describe('автосохранение в таблице', () => {
  ...
  test('правка уходит сама и переживает перезагрузку', async ({ page }) => {
    ...
    const cell = page.locator('.grid-tbl tbody tr').first().locator('input.cell').first()
    ...
    await expect(page.locator('[data-sync="saved"]')).toBeVisible({ timeout: 15_000 })
    // ушёл именно батч-запрос, и он ответил 2xx
    const calls = diag.since(mark).concat(diag.calls)
    expect(calls.some((c) => c.url.includes('/batch/save/') && c.status === 200)).toBeTruthy()
    expect(diag.failed).toEqual([])

    await page.reload()
    const again = page.locator('.grid-tbl tbody tr').first().locator('input.cell').first()
  ...
  test('кнопка «Сохранить» остаётся и работает', async ({ page }) => {
    ...
    const cell = page.locator('.grid-tbl tbody tr').nth(1).locator('input.cell').first()
    const scale = ['6.0', '6.5', '7.0', '7.5', '8.0']
    const current = await cell.inputValue()
    const value = scale.find((x) => x !== current)!
    await cell.fill(value)
    ...
    await expect
      .poll(() => diag.since(mark).filter((c) => c.url.includes('/batch/save/') && c.status === 200).length)
      .toBeGreaterThan(0)
    await expect(page.locator('[data-sync="saved"]')).toBeVisible()
    expect(diag.consoleErrors).toEqual([])
  })

  test('без связи правки копятся и уходят, когда связь вернулась', async ({ page, context }) => {
    const diag = watch(page)
    await page.goto('/table')

    const cell = page.locator('.grid-tbl tbody tr').nth(2).locator('input.cell').first()
    await expect(cell).toBeVisible()
    ...
    const value = scale.find((x) => x !== current)!
    await cell.fill(value)
    await expect(page.locator('[data-sync="offline"]')).toContainText('нет связи', { timeout: 15_000 })
    ...
    await page.reload()
    const again = page.locator('.grid-tbl tbody tr').nth(2).locator('input.cell').first()
    await expect(again).toHaveValue(value)
    expect(diag.consoleErrors).toEqual([])
  })
})
