...
async function setPasswordByLink(
  ...
  const fresh = await context.browser()!.newContext({ storageState: undefined })
  const page = await fresh.newPage()
  await page.goto('/login')
  await apiPost(page, '/api/auth/password/set/', { token, new_password: password })
  await fresh.close()
}


test.describe('вход', () => {
  test('верные почта и пароль пускают на экраны своей роли', async ({ page }) => {
    ...
    await expect(page.locator('.shell__role')).toContainText('Академический директор')
    expect(diag.failed).toEqual([])
  })

  test('неверный пароль объясняется, но не выдаёт, есть ли такой человек', async ({ page }) => {
    await page.goto('/login')
    await page.getByLabel('Почта', { exact: true }).fill(byKey('director_exam').email)
    await page.getByLabel('Пароль', { exact: true }).fill('точно-не-тот-пароль')
    await page.getByRole('button', { name: 'Войти', exact: true }).click()

    await expect(page.locator('.chip-risk')).toContainText('Неверная почта или пароль')
    await expect(page).toHaveURL(/\/login/)
  })

  test('на экране входа нет ни одного упоминания Microsoft', async ({ page }) => {
    await page.goto('/login')
    const text = await page.locator('body').innerText()
    expect(text).not.toMatch(/Microsoft|Entra|MSAL/i)
  })

  test('шесть неверных паролей подряд дают блокировку', async ({ page, context }) => {
    ...
    const admin = await context.browser()!.newContext({ storageState: statePath('admin') })
    ...
    for (let attempt = 1; attempt <= 5; attempt += 1) {
      ...
      expect(response.status(), `попытка ${attempt} должна отвечать 401`).toBe(401)
    }

    const sixth = await page.request.post('/api/auth/login/', { data: { email, password: 'мимо-6' } })
    expect(sixth.status()).toBe(429)
    expect((await sixth.json()).detail).toMatch(/попыток/)

    // даже верный пароль в блокировку не проходит
    const correct = await page.request.post('/api/auth/login/', { data: { email, password } })
...
test.describe('администратор заводит человека', () => {
  ...
  test('созданный пользователь ставит пароль по ссылке и входит', async ({ page, context }) => {
    test.setTimeout(120_000)
    const email = freshEmail()
    ...
    await expect(page.locator('.chip-ok').first()).toContainText(email)
    ...
    await expect(page.locator('.users__table tbody tr')).toContainText('пароль не задан')
    ...
    await invited.getByLabel('Новый пароль', { exact: true }).fill('Приглашённый!Спорт26')
    await invited.getByLabel('Ещё раз', { exact: true }).fill('Приглашённый!Спорт26')
  ...
  test('отключённый пользователь не входит, а запись остаётся', async ({ page, context }) => {
    test.setTimeout(120_000)
    const email = freshEmail()
    ...
    const found = await page.request.get(`/api/users/?search=${encodeURIComponent(email)}`)
    const list = await found.json()
    expect(Array.isArray(list) && list.length === 1, `поиск вернул ${JSON.stringify(list)}`).toBe(true)
    const id = list[0].id
    const off = await apiPatch<{ is_active: boolean }>(page, `/api/users/${id}/`, { is_active: false })
    expect(off.is_active).toBe(false)

    // отключённый не входит даже с верным паролем
    const other = await context.browser()!.newContext({ storageState: undefined })
    const denied = await other.request.post('/api/auth/login/', { data: { email, password } })
    expect([401, 403]).toContain(denied.status())
    await other.close()

    // запись на месте — на ней держится аудит
    const still = await (await page.request.get(`/api/users/?search=${encodeURIComponent(email)}`)).json()
...
test.describe('обязательная смена пароля', () => {
  test('до смены дальше экрана не пускают', async ({ page, context }) => {
    ...
    await page.getByLabel('Новый пароль', { exact: true }).fill('Свой!Собственный2026')
    await page.getByLabel('Ещё раз', { exact: true }).fill('Свой!Собственный2026')
