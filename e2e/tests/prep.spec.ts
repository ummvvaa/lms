...
test.describe('тренировка', () => {
  ...
  test('вопросы берутся из банка, разбор объясняет ошибки', async ({ page }) => {
    const diag = watch(page)
    await page.goto('/prep')
    await expect(page.locator('h1')).toContainText('Центр подготовки')

    const [started] = await Promise.all([
      page.waitForResponse((r) => r.url().includes('/api/prep/practice/start/')),
    ...
    const [finished] = await Promise.all([
      ...
      page.getByRole('button', { name: /Завершить и посмотреть разбор/ }).click(),
    ])
...
test.describe('пробный экзамен', () => {
  test('результат виден на графике и у Кымбат, текущий балл не меняется', async ({ browser }) => {
    ...
    const before = await (await student.request.get('/api/students/me/')).json()
    ...
    const mock = mocks.results.find((m: { exam_type: string }) => m.exam_type === 'IELTS')
    expect(mock, 'нужен пробный IELTS в справочнике').toBeTruthy()

    const run = await apiPost<{ id: number; run: number; questions: { answer_id: number; options: { id: number }[] }[] }>(
      student,
      `/api/prep/mocks/${mock.id}/start/`,
      {},
    )
    ...
    const attempts = await (await student.request.get('/api/attempts/?exam_type=IELTS&page_size=200')).json()
    const platform = attempts.results.filter((a: { source: string }) => a.source === 'platform')
    ...
    const directorContext = await browser.newContext({ storageState: statePath('director_exam') })
    ...
    await expect(list.locator('.chip-warn').first()).toContainText('ждёт решения')
  ...
  test('директор решает, учитывать ли балл', async ({ browser }) => {
    ...
    const mocks = await (await student.request.get('/api/prep/mocks/')).json()
    const mock = mocks.results.find((m: { exam_type: string }) => m.exam_type === 'IELTS')
    const run = await apiPost<{ id: number; questions: { answer_id: number; options: { id: number }[] }[] }>(
      student,
      `/api/prep/mocks/${mock.id}/start/`,
      {},
    )
    ...
    const review = await apiPost<{ score: number }>(student, `/api/prep/practice/${run.id}/finish/`, {})
    ...
    await expect
      .poll(
        async () => Number((await (await student.request.get('/api/students/me/')).json()).exam.ielts_current),
        { timeout: 10_000 },
      )
  ...
  test('слабые темы превращаются в задачи роадмапа', async ({ page }) => {
    ...
    expect(tasks.some((t: { title: string }) => t.title.includes('Подтянуть тему'))).toBe(true)
    await context.close()
  })
})
...
test.describe('банк заданий', () => {
  ...
  test('чужой директор банк не ведёт', async ({ browser }) => {
    const context = await browser.newContext({ storageState: statePath('director_sport') })
