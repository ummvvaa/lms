...
test("в зачёте классов нет строк учеников", async ({ browser }) => {
  ...
  const payload = (await (await student.request.get("/api/prep/quiz/")).json()) as {
    teams: { teams: Record<string, unknown>[] };
  };
  for (const row of payload.teams.teams) {
    expect(Object.keys(row).sort()).toEqual(["accuracy", "matches", "score", "team"]);
  }
});
...
test("закрытые бейджи видны с условием и прогрессом", async ({ browser }) => {
  ...
  await expect(student.locator(".badges__card--locked .num").first()).toContainText("из");
});
...
test("директор школы заводит бейдж", async ({ browser }) => {
  ...
  await expect(director.getByRole("heading", { name: "Достижения школы" })).toBeVisible();

  await director.getByRole("button", { name: "Добавить бейдж" }).first().click();
  await director.getByLabel("Код бейджа").fill("probe_browser_badge");
  await director.getByLabel("Название бейджа").fill(BADGE);
  await director.getByLabel("Что считает бейдж").selectOption({ label: "Решённые упражнения" });
...
test("уборка: бейдж прогона удалён", async ({ browser }) => {
  ...
  const listing = (await (await director.request.get("/api/badges/?page_size=100")).json()) as {
