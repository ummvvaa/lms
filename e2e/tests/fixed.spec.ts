...
test.describe("B4 · помощник: каждая кнопка что-то делает", () => {
  ...
  test("«Проверить баланс списка» считает по выбранному ученику", async ({
    page,
  }) => {
    ...
    await page
      .locator("select")
      .first()
      .selectOption(String(list.results[0].id));

    await expect
      .poll(() =>
        diag.calls.some(
          (c) => c.url.includes("/api/match/list-balance/") && c.status === 200,
        ),
      )
      .toBe(true);
    await expect(page.locator(".card").last()).toContainText(
      /reach|Список пуст/,
    );
  });
});
...
test.describe("I4 · чужой экран не открывается", () => {
  ...
  test("директора уводит с экранов ученика без ошибок в консоли", async ({
    page,
  }) => {
    const diag = watch(page);
...
test.describe("C1 · заголовок не двоится", () => {
  ...
  for (const [route, title] of [
    ...
    test(`${route}: «${title}» встречается один раз`, async ({ page }) => {
      ...
      expect(
        occurrences,
        `заголовок «${title}» напечатан ${occurrences} раза`,
      ).toBe(1);
    });
  }
});
...
test.describe("C2 · «Позже» в баннере запоминается", () => {
  ...
  test("баннер не возвращается при переходе на другой экран", async ({
    page,
  }) => {
    await page.goto("/dashboard");
