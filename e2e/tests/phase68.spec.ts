 ...
 * 3. У ученика цели вынесены в отдельную карточку «Цели поступления»,
 *    а в блоке «Поступление» остались только колонки таблицы.
 *
...
test("карточка куратора: блок «Поступление» ровно по таблице", async ({
  ...
  for (const label of [
    ...
    "Пароль от почты",
    "Пароль Common App",
    "Документы из таблицы поступления",
    "Паспорт",
    "Транскрипт",
...
test("у ученика цели вынесены из блока в свою карточку", async ({
  browser,
}) => {
  ...
  const goals = page
    .locator(".card")
    .filter({ hasText: "Цели поступления" })
    .first();
  await expect(goals).toBeVisible();
  await expect(goals).toContainText("Страна");
  await expect(goals).toContainText("Специальность");
