...
test("директор пишет памятку с экрана", async ({ browser }) => {
  ...
  await expect(director.getByRole("heading", { name: /Ресурсы/ })).toBeVisible();

  await director.getByRole("button", { name: "Добавить материал" }).first().click();
  ...
  await director.getByLabel("Короткое описание").fill("Как готовиться к пробному экзамену");
  await director.getByLabel("Текст материала").fill("Первый абзац памятки.\nВторой абзац памятки.");
...
test("ученик находит памятку по категории, читает и отмечает прочитанной", async ({ browser }) => {
  ...
  await expect(student.getByRole("button", { name: "Снять отметку" })).toBeVisible();
});
...
test("директор школы ведёт анкету профтеста", async ({ browser }) => {
  ...
  await expect(director.getByRole("heading", { name: "Вопросы профтеста" })).toBeVisible();
  // шесть посеянных вопросов на экране
  await expect(director.getByText("Какие школьные предметы вам нравятся больше всего?")).toBeVisible();
});
...
test("ученик проходит анкету: разбор из справочника либо честное «недоступно»", async ({ browser }) => {
  ...
  const catalog = (await (await student.request.get("/api/programs/?page_size=500")).json()) as {
    results: { id: number }[];
  };
  const known = new Set(catalog.results.map((row) => row.id));
  for (const direction of run.directions) {
    for (const program of direction.programs) expect(known.has(program.id)).toBeTruthy();
  }
});
...
test("уборка: памятка прогона удалена", async ({ browser }) => {
  ...
  const listing = (await (await director.request.get("/api/resources/?page_size=200")).json()) as {
