...
test.afterAll(async ({ browser }) => {
  ...
  const students = await (
    await page.request.get("/api/students/?search=Удалимова&page_size=100")
  ).json();
  ...
  const universities = await (
    await page.request.get(
      "/api/universities/?search=Школьный вуз&page_size=100",
    )
  ).json();
...
test.describe("удаление ученика и возврат из архива", () => {
  ...
  test("карточка уходит в архив и возвращается оттуда", async ({ page }) => {
    ...
    await expect(page.locator(".confirm__title")).toContainText(
      "Удалить «Удалимова",
    );
    ...
    await expect
      .poll(
        () =>
          diag
            .since(mark)
            .filter((c) => c.method === "DELETE" && c.status === 200).length,
      )
      .toBeGreaterThan(0);
    ...
    const list = await (
      await page.request.get(`/api/students/?search=delete.${suffix}`)
    ).json();
    ...
    const tasks = await (
      await page.request.get(`/api/tasks/?student=${student.id}`)
    ).json();
...
test.describe("чужой домен удалить нельзя", () => {
  ...
  test("директор экзаменов не убирает активность директора талантов", async ({
    page,
    browser,
  }) => {
    ...
    const adminCsrf = (await admin.cookies()).find(
      (c) => c.name === "csrftoken",
    )!.value;

    const talent = await browser.newContext({
      storageState: statePath("director_talent"),
    });
    const talentPage = await talent.newPage();
    await talentPage.goto("/dashboard");
    const talentCsrf = (await talent.cookies()).find(
      (c) => c.name === "csrftoken",
    )!.value;
    ...
    await adminPage.request.delete(`/api/students/${student.id}/`, {
    ...
    await expect
      .poll(
        () =>
          diag
            .since(mark)
            .filter((c) => c.url.includes("/revert/") && c.status === 200)
            .length,
      )
      .toBeGreaterThan(0);
