...
test.describe('квиз знакомства', () => {
  ...
  test('вопросы идут по одному, прогресс сохраняется по шагам', async ({ page }) => {
    ...
    await expect(page.locator('.onboarding__count')).toContainText('Вопрос 1 из 8')
    ...
    await expect(page.locator('.onboarding__count')).toContainText('Вопрос 2 из 8')
  ...
  test('квиз можно отложить', async ({ page }) => {
    ...
    const [skipped] = await Promise.all([
      ...
      page.getByRole('button', { name: 'Пропустить и вернуться позже' }).click(),
    ])
    expect(skipped.status()).toBe(200)
    await page.waitForURL(/\/dashboard/)
  })

  test('ответ доезжает до профиля и переживает перезагрузку', async ({ page }) => {
    await resetQuiz(page)
    await apiPost(page, '/api/onboarding/answer/', { question: 'english_score', value: '6.5' })
...
test.describe('ответы ученика ждут подтверждения', () => {
  test('Кымбат видит балл ученика отдельным списком и подтверждает', async ({ browser }) => {
    ...
    await apiPost(student, '/api/onboarding/answer/', { question: 'english_score', value: '7.0' })
    ...
    const [confirmed] = await Promise.all([
      director.waitForResponse((r) => r.url().includes('/api/onboarding/pending/')),
  ...
  test('в журнале видно, что число назвал ученик', async ({ browser }) => {
    ...
    await student.goto('/dashboard')
    await resetQuiz(student)
    await apiPost(student, '/api/onboarding/answer/', { question: 'gpa', value: '3.7' })
    ...
    await director.getByRole('button', { name: 'История изменений' }).click()

    await expect(director.locator('table.history')).toContainText('анкета')
    await directorContext.close()
  })
})
...
test.describe('XP и стрик', () => {
  ...
  test('на дашборде видны уровень, стрик и XP каждой задачи', async ({ page }) => {
    await page.goto('/dashboard')
    const panel = page.locator('.today')
  ...
  test('никаких рейтингов и сравнения с другими', async ({ page }) => {
    ...
    for (const word of ['рейтинг', 'место в', 'лидер', 'лучше, чем']) {
      expect(text.toLowerCase(), `на дашборде появилось сравнение: ${word}`).not.toContain(word)
    }
  })
  ...
  test('у ученика без стрика формулировка поддерживающая', async ({ page }) => {
    ...
    await expect(page.locator('.today__phrase')).toContainText(state.streak_phrase)
  })
})
