...
test("удаление навсегда: запись с журналом и ученик с полным набором", async ({
  browser,
}) => {
  // фаза 67: модалку удаления надо показать на чём-то настоящем. Заводим
  // пользователя, который успел наследить в журнале, и убеждаемся, что
  // у одного из учеников есть всё сразу — документы, попытки, пароли,
  // заметки: иначе числа в модалке нечем проверять глазами
  const admin = await as(browser, "admin");
  const everyone = await students(admin);

  const row = everyone.find((r) => r.email === probeEmail("pupil01"));
  expect(row, "ученик посева на месте").toBeTruthy();

  const card = (await (
    await admin.request.get(`/api/students/${row!.id}/`)
  ).json()) as { id: number };
