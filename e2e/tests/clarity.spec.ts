...
test.describe('первый вход', () => {
  ...
  test('три шага показываются, прячутся и вызываются заново', async ({ page }) => {
...
test.describe('панель «Начало работы»', () => {
  ...
  test('строка чеклиста ведёт на свой экран', async ({ page }) => {
    ...
    const step = panel.locator('.start__step').filter({ hasText: 'Вузы заведены' })
    ...
    await page.goto('/dashboard')
    await page.locator('.start').getByRole('button', { name: 'Свернуть' }).click()
    await expect(page.locator('.start__list')).toHaveCount(0)
    await page.reload()
    await expect(page.locator('.start__list')).toHaveCount(0)
    await page.locator('.start').getByRole('button', { name: 'Развернуть' }).click()
...
test.describe('пустой экран объясняет себя', () => {
  ...
  test('поиск, который никого не нашёл, предлагает снять фильтры', async ({ page }) => {
    const diag = watch(page)
    await page.goto('/table')

    await page.getByPlaceholder('Поиск по имени').fill('такого-человека-нет-зззз')
...
test.describe('ошибка в файле объясняется по-человечески', () => {
  ...
  test('одна кривая строка не отменяет файл', async ({ page }) => {
    ...
    const list = await (await page.request.get('/api/students/?page_size=3')).json()
    ...
    await apply.click()
    await expect
      .poll(() => diag.since(mark).filter((c) => c.url.includes('/import/apply/') && c.status === 200).length)
      .toBeGreaterThan(0)
