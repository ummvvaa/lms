...
async function ensureUser(admin: Page, email: string, fullName: string): Promise<Managed> {
  const list = (await (await admin.request.get("/api/users/?search=" + encodeURIComponent(email))).json()) as
    | Managed[]
    | { results: Managed[] };
  const rows = Array.isArray(list) ? list : list.results;
  const found = rows.find((row) => row.email === email);
  if (found) return found;
  return apiPost<Managed>(admin, "/api/users/", { email, full_name: fullName, role: "director_talent" });
}

async function issuePassword(admin: Page, user: Managed): Promise<string> {
  const issued = await apiPost<{ password: string }>(admin, `/api/users/${user.id}/temp-password/`, {});
  return issued.password;
}

test("D1: смена пароля десять раз подряд — сессия ни разу не теряется", async ({ browser }) => {
  const admin = await as(browser, "admin");
  const user = await ensureUser(admin, probeEmail("p36.change"), "Айгерим Прогон");
  ...
  for (let round = 1; round <= 10; round += 1) {
    ...
    await expect(page.getByRole("heading", { name: "Смените пароль" })).toBeVisible();
    ...
    await Promise.all([
      page.waitForResponse((r) => r.url().includes("/api/auth/password/change/") && r.status() === 200),
    ...
    const denied = diag.since(mark).filter((c) => c.status === 401 || c.status === 403);
...
test("D2: блокировка объясняет, когда и к кому; администратор снимает её кнопкой", async ({
  ...
  await apiPost(admin, "/api/auth/locks/unlock/", { scope: "account", value: user.email });
  ...
  await expect(refusal).toContainText("Слишком много попыток входа в эту учётную запись");
  ...
  const locks = admin.locator(".datacard").filter({ hasText: "Блокировки входа" });
  ...
  await Promise.all([
    admin.waitForResponse((r) => r.url().includes("/api/auth/locks/unlock/") && r.status() === 200),
    row.getByRole("button", { name: "Снять блокировку" }).click(),
  ]);
  await expect(locks.locator(".locks__row").filter({ hasText: user.email })).toHaveCount(0);
...
test("D3: бэкенд остановлен — полоса «нет связи», экран и черновик на месте; поднят — работа продолжается", async ({
  ...
  const list = await (await page.request.get("/api/students/?page_size=1&ordering=id")).json();
  const studentId = list.results[0].id as number;
  const before = (await (await page.request.get(`/api/profiles/exam/${studentId}/`)).json())
  ...
  try {
    ...
    await expect(page.locator(".connection")).toContainText("Нет связи с сервером");
    ...
    await expect(page.locator("[data-sync]")).toHaveAttribute("data-sync", /offline|dirty|saving/);

    compose("start", "backend");
    stopped = false;
    await expect(page.locator(".connection")).toHaveCount(0, { timeout: 90_000 });
    // черновик дослан сам, без перезагрузки и без входа
    await expect(page.locator("[data-sync]")).toHaveAttribute("data-sync", "saved", { timeout: 30_000 });
    expect(page.url()).toContain("/table");
    const saved = await (await page.request.get(`/api/profiles/exam/${studentId}/`)).json();
    ...
    await expect
      .poll(async () => (await page.request.get("/api/auth/me/").catch(() => null))?.status() ?? 0, {
        timeout: 90_000,
      })
      .toBe(200);
  }
