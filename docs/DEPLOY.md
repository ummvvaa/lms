...
- SMTP-доступ для писем со ссылками на вход
- Доступы Microsoft Entra ID: `tenant_id`, `client_id`, GUID групп для ролей
...
**Вход через Microsoft не работает.** При пустых `ENTRA_TENANT_ID` и
`ENTRA_CLIENT_ID` кнопка выключена намеренно — работает вход по ссылке
на личную почту.
