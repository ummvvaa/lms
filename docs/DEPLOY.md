...
запускаться при `DEBUG=0`. То же с `create_probe_users` — одноразовыми
записями браузерного прогона: в бою они не заводятся, а забытая запись
`*@probe.local` в бою не входит ни паролем, ни по ссылке. `purge_probe_users`
работает в любом режиме — уборка должна быть возможна всегда. Команды
наполнения выдуманными данными удалены из кода целиком в фазе 22. Учётные записи заводит администратор на экране
...
Нехранение запросов на стороне провайдера — условие учётной записи
(zero data retention), а не заголовок запроса: включить его из кода
нельзя, договариваться нужно с провайдером. `LLM_NO_RETENTION=1` —
признак того, что это оговорено. В модель уходят только поля, нужные операции;
